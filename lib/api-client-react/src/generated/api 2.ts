...
/**
 * Admin/provider flow after `POST /v1/user/create` (or createExternal). Proxies to the remote API.

 * @summary Register a technician profile for an existing user (proxied to hayyah v1)
 */
export const getAssignTechnicianToUserUrl = (userId: string) => {
  return `/api/users/${userId}/technician`;
};

export const assignTechnicianToUser = async (
  userId: string,
  assignTechnicianToUser: AssignTechnicianToUser,
  options?: RequestInit,
): Promise<Technician> => {
  return customFetch<Technician>(getAssignTechnicianToUserUrl(userId), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(assignTechnicianToUser),
  });
};

export const getAssignTechnicianToUserMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof assignTechnicianToUser>>,
    TError,
    { userId: string; data: BodyType<AssignTechnicianToUser> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["assignTechnicianToUser"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return assignTechnicianToUser(userId, data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type AssignTechnicianToUserMutationResult = NonNullable<
  Awaited<ReturnType<typeof assignTechnicianToUser>>
>;
export type AssignTechnicianToUserMutationBody =
  BodyType<AssignTechnicianToUser>;
export type AssignTechnicianToUserMutationError = ErrorType<unknown>;

/**
 * @summary Register a technician profile for an existing user (proxied to hayyah v1)
 */
export const useAssignTechnicianToUser = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getAssignTechnicianToUserMutationOptions(options));
};
...
export const getListTechniciansQueryKey = () => {
  return [`/api/v1/technicians`] as const;
};
